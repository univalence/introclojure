...
(defn exercice-form? [fzip]
  (and (= :list (z/tag fzip))
       (= 'exercice (-> fzip z/down z/value))))


(defn parse-exercice-form [fzip current tags]

  [[{:type :exercice :content (-> fzip z/down z/remove z/sexpr)}] current tags]
...
(defn parse-content-single
  ...
  (cond (:fact-arrow current)
        ...
        (cond (ns-form? fzip)
              ...
              (exercice-form? fzip)
              (parse-exercice-form fzip current tags)
